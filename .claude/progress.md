...
| 2026-07-23 | Updated Optimeleon loader URLs in `app/layout.tsx` to ID `vy2rwmF0nv3Q` on `http://localhost:8787`. Paste carried a Cloudflare-tunnel host (`population-promotes-cassette-listed.trycloudflare.com`) but user instructed to use `localhost:8787` instead, keeping the pasted ID. | tsc ✅ (exit 0), lint ✅ ("No ESLint warnings or errors"), build ✅ (10/10 pages) | Inline code byte-identical; host override was an explicit user instruction, an exception to the "host in paste is intentional" rule. |
